...
namespace NaughtyAttributes.Editor
{
    public abstract class PropertyDrawerBase : PropertyDrawer
    {
        ...
        protected void DrawLineProperties(Rect position, SerializedProperty property,string[] propertyNames) {
            var positionX = position.x;
            var oneFieldWidth = position.width / propertyNames.Length;
            foreach (var name in propertyNames) {
                DrawProperty(position, property, name, positionX, oneFieldWidth);
                positionX += oneFieldWidth;
            }
        }
        protected void DrawProperty(Rect position, SerializedProperty property, string propertyName, float x,
            float width) {
            var health = property.FindPropertyRelative(propertyName);
            var healthContent = GUIContent.none;
            var healthRect = new Rect(x, position.y + EditorGUIUtility.singleLineHeight, width,
                EditorGUIUtility.singleLineHeight);
            EditorGUI.PropertyField(healthRect, health, healthContent);
        }
